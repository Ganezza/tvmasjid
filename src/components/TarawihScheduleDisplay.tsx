...
const TarawihScheduleDisplay: React.FC = () => {
  ...
  const fetchTarawihSchedule = useCallback(async () => {
    ...
    try {
      ...
      if (!ramadanModeStatus) {
        setIsLoading(false);
        return; // Do not fetch or display if Ramadan mode is not active
      }

      // 2. Fetch Tarawih schedule for today
      const currentDayOfWeek = getIndonesianDayOfWeek(dayjs());
  ...
  useEffect(() => {
    ...
    // Update every minute to ensure the day of week is accurate
    const interval = setInterval(fetchTarawihSchedule, 60 * 1000); 
  ...
  if (!isRamadanModeActive) {
    return null; // Don't render anything if Ramadan mode is not active
  }
    ...
    return (
      <div className="bg-gray-800 bg-opacity-70 p-6 rounded-xl shadow-2xl w-11/12 max-w-4xl text-center mb-8 text-white">
        <p className="text-xl text-gray-400">
          Jadwal Tarawih untuk hari ini tidak ditemukan.
        </p>
      </div>
    );
  }
