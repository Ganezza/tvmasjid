...
const FinancialDisplay: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="bg-gray-800 bg-opacity-70 p-6 rounded-xl shadow-2xl w-11/12 max-w-4xl text-center mb-8 text-white">
        <p className="text-2xl lg:text-3xl xl:text-4xl">Memuat informasi keuangan...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-800 bg-opacity-70 p-6 rounded-xl shadow-2xl w-11/12 max-w-4xl text-center mb-8">
      <h3 className="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold mb-3 text-yellow-300">
        Informasi Keuangan Masjid
      </h3>
      <p className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-green-400 mb-2">
        Saldo Kas: Rp {totalBalance.toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
      </p>
      <p className="text-lg md:text-xl lg:text-2xl xl:text-3xl text-gray-300 mb-4">
        Data per: <span className="font-semibold">{lastFridayDate}</span>
      </p>

      <h4 className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold mb-3 text-blue-300">
        Rincian Transaksi Terbaru
      </h4>
      {recentRecords.length === 0 ? (
        <p className="text-gray-400 text-lg md:text-xl lg:text-2xl xl:text-3xl">Belum ada transaksi yang tercatat.</p>
      ) : (
        <AutoScrollingFinancialRecords heightClass="h-48 md:h-64 lg:h-80 xl:h-96">
          <div className="space-y-3">
            {recentRecords.map((record) => (
              <div key={record.id} className="flex flex-col items-start bg-gray-700 p-3 rounded-md shadow-sm text-left">
                <p className="font-medium text-xl md:text-2xl lg:text-3xl xl:text-4xl text-blue-200">
                  {record.description}
                </p>
                <p className={`text-lg md:text-xl lg:text-2xl xl:text-3xl font-semibold ${record.transaction_type === "inflow" ? "text-green-400" : "text-red-400"}`}>
                ...
                <p className="text-xs md:text-sm lg:text-base xl:text-lg text-gray-400">
